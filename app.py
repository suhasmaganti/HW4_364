...
search_gifs = db.Table('search_gifs',db.Column('gif_id', db.Integer, db.ForeignKey('Gifs.id')),db.Column('search_id',db.Integer, db.ForeignKey('SearchTerms.id')))
...
class User(UserMixin, db.Model):
    ...
    #gif_collection_id = db.Column(db.Integer,db.ForeignKey("PersonalGifCollections.id"))
    personal_collections = db.relationship('PersonalGifCollection',secondary='user_collection', backref=db.backref('Users',lazy='dynamic'),lazy='dynamic') 
...
class Gif(db.Model):
    ...
    title = db.Column(db.String(128), index=True)   # title (String up to 128 characters)
    embedURL = db.Column(db.String(256), index=True)    # embedURL (String up to 256 characters)
    personal_collections = db.Column(db.Integer,db.ForeignKey("PersonalGifCollections.id"))
...
class PersonalGifCollection(db.Model):
    ...
    #gif_collections = db.relationship('User',backref='PersonalGifCollection')       
    ...
    gifs = db.relationship('Gif',backref=db.backref('PersonalGifCollections'),lazy='dynamic')
...
class SearchTerm(db.Model):
    ...
    gifs_searched = db.relationship('Gif',secondary=search_gifs,backref=db.backref('SearchTerms',lazy='dynamic'),lazy='dynamic')
...
def get_gifs_from_giphy(search_string):
    ...
    if form.validate_on_submit():
        ...
        baseurl = "https://api.giphy.com/v1/gifs/search"
        params = {"api_key":api_key, "q":search, "limit":5}
...
def get_or_create_search_term(db_session,term):
    ...
    if searchTerm:
        ...
        print(gif_list)
        for gif in gif_list[0]['data']:
            gif = get_or_create_gif(db_session=db_session,title=gif['title'],url=gif['embed_url'])
            searchTerm.gifs_searched.append(gif)
...
def search_results(search_term):
    ...
    relevant_gifs = term.gifs_searched.all()
    #print(len(relevant_gifs))
...
def create_collection():
    ...
    #Then, use the information available to you at this point in the function 
    #(e.g. the list of gif objects, the current_user) to invoke the get_or_create_collection function, 
    #and redirect to the page that shows a list of all your collections.
    # If the form is not validated, this view function should simply render the create_collection.html template 
    #and send the form to the template.
    if form.validate_on_submit():
        selected_gifs = form.gif_picks.data
        obj = [get_gif_by_id(int(i)) for i in selected_gifs]
        collections = get_or_create_collection(db_session=db.session,name=form.name.data,current_user=current_user,gif_list=obj)
...
def collections():
    # Replace with code
    # TODO 364: This view function should render the collections.html template so that 
    #only the current user's personal gif collection links will render in that template. 
